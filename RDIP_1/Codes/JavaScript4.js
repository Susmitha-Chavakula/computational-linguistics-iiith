...
var stoplist = ['were','are','is','had','found','grown','ate','me','heard','this','himself','him','his','does','did','ran'];
...
function checkVal() {
	...
	while(i!=uniqueword.length) {
		...
		if(arr[uniqueword.indexOf(k)]==0){
			...
			uniquewordcount++;
			if(stoplist.indexOf(k)!=-1){
				stemarr[j]=stoplist1[stoplist.indexOf(k)];
...
function getstemans() {
	...
	if(userstemVal===stemwordcount){
		...
		document.getElementById('finalstatus').innerHTML = "<p style='color:green'>Right Answer</p>";
	}
	else{
		document.getElementById('newtype').style.backgroundColor = "red";
		document.getElementById('finalstatus').innerHTML = "<p style='color:red'>Wrong Answer</p>";
	}
